...
if ( $image {
		0
	} != '/' || strpos( dirname( $image ), ':' ) || preg_match( '/(\.\.|<|>)/', $image ) ) {
	/*header('HTTP/1.1 400 Bad Request');
...
if ( isset( $_GET[ 'crop' ] ) ) {
	$current_ratio = round( $width / $height, 2 );
	$desired_ratio_after = round( $maxWidth / $maxHeight, 2 );
	$desired_ratio_before = round( $maxHeight / $maxWidth, 2 );
	/**
	 ...
	 */
	$offsetX = floor( ( ( $new_width - $maxWidth ) / 2 ) * $width_ratio );
	$offsetY = floor( ( ( $new_height - $maxHeight ) / 2 ) * $height_ratio );
}
