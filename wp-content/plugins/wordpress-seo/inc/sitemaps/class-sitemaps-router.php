...
class WPSEO_Sitemaps_Router {
	...
	public function init() {
		...
		add_rewrite_rule( 'sitemap_index\.xml$', 'front-page.php?sitemap=1', 'top' );
		add_rewrite_rule( '([^/]+?)-sitemap([0-9]+)?\.xml$', 'front-page.php?sitemap=$matches[1]&sitemap_n=$matches[2]', 'top' );
		add_rewrite_rule( '([a-z]+)?-?sitemap\.xsl$', 'front-page.php?xsl=$matches[1]', 'top' );
	}
	...
	public static function get_base_url( $page ) {
		...
		$base = $wp_rewrite->using_index_permalinks() ? 'front-page.php/' : '/';
